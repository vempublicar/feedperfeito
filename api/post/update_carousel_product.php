...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $existingProduct = $carouselProduct->find($id);
    $imagesInDatabase = [];
 ...
 $resultado = $carouselProduct->update($id, $productData);
     if ($resultado) {
        ...
        $_SESSION['status_message'] = 'Produto Carrossel atualizado com sucesso!';
        header('Location: ' . $_SESSION['base_url'] . '/admin/produtos-carrossel');
        exit();
    } else {
        $_SESSION['status_type'] = 'error';
        $_SESSION['status_message'] = 'Erro ao atualizar o Produto Carrossel.';
        header('Location: ' . $_SESSION['base_url'] . '/admin/produtos-carrossel');
        exit();
    }
} else {
