...
$multipleProducts = [];
$cacheKey = 'multiple_products_cache';
$cacheTimestampKey = 'multiple_products_cache_timestamp';
...
if (isset($_SESSION[$cacheKey]) && isset($_SESSION[$cacheTimestampKey]) && (time() - $_SESSION[$cacheTimestampKey] < $cacheDuration)) {
    $multipleProducts = $_SESSION[$cacheKey];
} else {
    ...
    $_SESSION[$cacheKey] = $multipleProducts;
    $_SESSION[$cacheTimestampKey] = time();
}

// Catálogo (virá do banco futuramente)
$catalogo = $multipleProducts; // Usar os produtos do banco de dados
...
<section id="loja" class="py-12 bg-gray-50">
    <div class="container mx-auto px-4">
        <div class="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end flex-wrap gap-4">
            <div>
                <h2 class="text-3xl font-bold text-black mb-2">
                    Loja de <span class="font-light">Serviços</span>
                </h2>
                <div class="w-20 h-1 bg-black mb-4"></div>
                <p class="text-gray-600">Troque seus créditos por serviços prontos para personalizar.</p>
            </div>
            ...
            <div class="flex items-center">
                <label class="text-gray-600 mr-2">Utilização:</label>
                <select id="filtroUtilization"
                    class="px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent">
                    <option value="">Todas</option>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6" id="gradeLoja">
...
<div id="productOffcanvas"
    ...
    <div class="flex flex-col h-full">
        <div class="flex justify-between items-center p-4 border-b">
            <h2 class="text-2xl font-bold" id="offcanvas-product-name">Nome do Serviço</h2>
        ...
        <div class="flex-1 overflow-y-auto p-4">
            ...
            <p id="offcanvas-product-description" class="text-gray-700 text-base mb-4">Descrição do serviço.</p>
...
<script>
    const allMultipleProducts = <?= json_encode($multipleProducts); ?>;
</script>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function filterProducts() {
            ...
            const filteredProducts = allMultipleProducts.filter(p => {
                const matchesCategory = categoryFilter === '' || p.category === categoryFilter;
                const matchesTheme = themeFilter === '' || p.theme === themeFilter;
                const matchesType = typeFilter === '' || p.type === typeFilter;
                const matchesUtilization = utilizationFilter === '' || p.utilization.toLowerCase().includes(utilizationFilter.toLowerCase());
                const matchesSearch = p.name.toLowerCase().includes(searchFilter) ||
                                      ...
                                      p.utilization.toLowerCase().includes(searchFilter) ||
                                      (JSON.parse(p.customization_types) || []).some(type => type.toLowerCase().includes(searchFilter));
                return matchesCategory && matchesTheme && matchesType && matchesUtilization && matchesSearch;
            });
        ...
        const purchaseForm = document.getElementById('purchaseForm'); // Já existe, mas o formulário não tinha o ID
        ...
        renderProducts(allMultipleProducts);
